...
import * as OpenStellar from '../../../openstellar_module';
import * as StellarSdk from '@stellar/stellar-sdk';

export { BountyStatus, WorkStatus } from '../../../openstellar_module';
...
export const ContractProvider = ({ children }) => {
    ...
    const CONTRACT_ID = OpenStellar.networks.futurenet.contractId;
    ...
    async function executeTransaction(operation, baseFee) {
        ...
        const transaction0 = new StellarSdk.TransactionBuilder(sourceAcc, {
            fee: (baseFee === undefined || baseFee === '') ? StellarSdk.BASE_FEE : baseFee,
            networkPassphrase: StellarSdk.Networks.FUTURENET,
        })
            .addOperation(operation)
            .setTimeout(StellarSdk.TimeoutInfinite /* 30 */)
            .build();
    ...
    const approveToken = async (from, spender, payAmount) => {
        const tokenContract = new StellarSdk.Contract(DEF_PAY_TOKEN);
        const res = await executeTransaction(
            tokenContract.call('approve', 
                new StellarSdk.Address(from).toScVal(), // from
                new StellarSdk.Address(spender).toScVal(), // spender
                StellarSdk.nativeToScVal(Number(payAmount * 2), { type: 'i128' }), // double payAmount for fee
                StellarSdk.xdr.ScVal.scvU32(535680) // expiration_ledger
            ),
        );
    ...
    const createBounty = async (creator, name, reward, payToken, deadline) => {
        const res = await executeTransaction(
            contract.call('create_bounty', 
                ...
                StellarSdk.xdr.ScVal.scvU64(new StellarSdk.xdr.Uint64(reward)), 
                new StellarSdk.Address(payToken).toScVal(), 
                StellarSdk.xdr.ScVal.scvU64(new StellarSdk.xdr.Uint64(deadline))
            ),
            '1000000'
        );
    ...
    const applyBounty = async (participant, bountyId) => {
        const res = await executeTransaction(
            contract.call('apply_bounty', 
                new StellarSdk.Address(participant).toScVal(), 
                StellarSdk.xdr.ScVal.scvU32(bountyId)
            )
        );
    ...
    const submitWork = async (participant, workId) => {
        const res = await executeTransaction(
            contract.call('submit_work', 
                new StellarSdk.Address(participant).toScVal(), 
                StellarSdk.xdr.ScVal.scvU32(workId)
            )
        );
    ...
    const tokenBalances = async (account, token) => {
		const contract2 = new OpenStellar.Contract({contractId: CONTRACT_ID, 
            networkPassphrase: OpenStellar.networks.futurenet.networkPassphrase, 
